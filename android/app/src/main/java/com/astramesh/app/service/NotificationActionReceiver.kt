...
class NotificationActionReceiver : BroadcastReceiver() {
    companion object {
        const val ACTION_REPLY = "com.astramesh.app.ACTION_REPLY"
        const val ACTION_MARK_READ = "com.astramesh.app.ACTION_MARK_READ"
        const val ACTION_MUTE = "com.astramesh.app.ACTION_MUTE"
        const val EXTRA_REPLY = "extra_reply"
    }

    override fun onReceive(context: Context, intent: Intent) {
        val contactKey = intent.getStringExtra("contactKey") ?: return
        val service = AstraMeshService.getInstance() ?: return
        val db = service.db
